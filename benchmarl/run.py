...
def _build_callbacks(algorithm_name: str, cfg: DictConfig):
    """Trả về danh sách callback instances phù hợp với algorithm."""
    callbacks = []
    ...
    if nashconv_cfg is not None and OmegaConf.select(nashconv_cfg, "enable", default=False):
        ...
        callbacks.append(
            NashConvCallback(
                ...
                deterministic_eval=OmegaConf.select(nashconv_cfg, "deterministic_eval", default=True),
                obs_key=OmegaConf.select(nashconv_cfg, "obs_key", default="observation"),
            )
        )
        print(f"[NashConv] enabled  (br_updates={nashconv_cfg.get('br_updates', 5)}, "
              f"eval_interval={nashconv_cfg.get('eval_interval', 1)})")
...
def hydra_experiment(cfg: DictConfig) -> None:
    ...
    callbacks = _build_callbacks(algorithm_name, cfg)
    if callbacks:
