...
from benchmarl.optimizers import AdamConfig
...
class Experiment(CallbackNotifier):
    ...
    Args:
        ...
        optimizer_config (OptimizerConfig, optional): the optimizer configuration.
            If None, it defaults to :class:`~benchmarl.optimizers.AdamConfig`,
            which reads ``lr``, ``adam_eps`` and ``adam_extra_kwargs`` from
            ``config`` and so reproduces the previous behaviour exactly.
    ...
    def _setup_algorithm(self):
        ...
        self.optimizers = {
            group: {
                loss_name: self.optimizer_config.get_optimizer(params, self.config)
                for loss_name, params in self.algorithm.get_parameters(group).items()
            }
            for group in self.group_map.keys()
        }
        self.two_point_optimizers = self.optimizer_config.requires_two_gradient_evals()
        if self.optimizer_config.uses_gradient_as_operator():
            self._check_gradient_is_not_clipped()
        if self.two_point_optimizers:
            self._check_two_point_optimizers()
        self._check_sync_period()

    def _updates_per_collection_round(self) -> int:
        return self.config.n_optimizer_steps(self.on_policy) * -(
            -self.config.train_batch_size(self.on_policy)
            // self.config.train_minibatch_size(self.on_policy)
        )

    def _check_sync_period(self):
        """Warn if a periodic optimizer's cycle straddles the collection boundary."""
        updates = self._updates_per_collection_round()
        for optimizers in self.optimizers.values():
            for optimizer in optimizers.values():
                k = getattr(optimizer, "k", None)
                if k is None or updates % k == 0:
                    continue
                warnings.warn(
                    f"{type(optimizer).__name__} syncs every k={k} updates, but a "
                    f"collection round runs {updates} of them ({updates % k} left "
                    f"over). Data will be collected with fast weights mid-cycle, "
                    f"while the paper treats the slow weights as the model. Pick a "
                    f"k that divides {updates}, or set sync_at_round_end=True."
                )
                return

    def _check_gradient_is_not_clipped(self):
        """Optimizers that treat the gradient as an operator cannot be clipped."""
        if self.config.clip_grad_val is not None:
            raise ValueError(
                f"{type(self.optimizer_config).__name__} treats the gradient as the "
                f"operator F of a monotone-operator method. Gradient clipping distorts "
                f"F (a clipped gradient is not the gradient of anything) and corrupts "
                f"the local Lipschitz estimate that drives the adaptive step. Set "
                f"experiment.clip_grad_val=null to run it, and say so in the write-up. "
                f"Got clip_grad_val={self.config.clip_grad_val}."
            )

    def _check_two_point_optimizers(self):
        """Preconditions for optimizers that evaluate the gradient at a probe point."""
        for group, optimizers in self.optimizers.items():
            seen = {}
            for loss_name, optimizer in optimizers.items():
                for param_group in optimizer.param_groups:
                    for prm in param_group["params"]:
                        if id(prm) in seen:
                            raise ValueError(
                                f"Parameter shared between losses "
                                f"'{seen[id(prm)]}' and '{loss_name}' of group "
                                f"'{group}'. A two-point optimizer moves every "
                                f"parameter to its own probe point before the second "
                                f"backward pass, which is only well defined when the "
                                f"losses do not share parameters (e.g. a shared "
                                f"actor-critic trunk)."
                            )
                        seen[id(prm)] = loss_name
    ...
    def _optimizer_name(self) -> str:
        """Short name of the optimizer, for the run folder.

        Without this, an IPPO+Adam run and an IPPO+PCVI run would produce folders
        with identical names, which makes an optimizer ablation impossible to read
        back off disk.
        """
        name = type(self.optimizer_config).__name__.lower()
        if name.endswith("config"):
            name = name[: -len("config")]
        inner = getattr(self.optimizer_config, "inner", None)
    ...
    def _setup_name(self):
        ...
        if self.config.restore_file is None:
            self.name = generate_exp_name(
                f"{self.algorithm_name}_{self.optimizer_name}"
                f"_{self.task_name}_{self.model_name}",
                ""
            )
    ...
    def _collection_loop(self):
        ...
        for _ in range(
            ...
            for group in self.train_group_map.keys():
                ...
                for optimizer in self.optimizers[group].values():
                    if hasattr(optimizer, "reset_lambda"):
                        optimizer.reset_lambda()

                optimizer_loop = (
                    self._optimizer_loop_two_point
                    if self.two_point_optimizers
                    else self._optimizer_loop
                )
                training_tds = []
                for _ in range(self.config.n_optimizer_steps(self.on_policy)):
                    for _ in range(
                        ...
                        training_tds.append(optimizer_loop(group))
                for optimizer in self.optimizers[group].values():
                    if hasattr(optimizer, "sync_now"):
                        optimizer.sync_now()
    ...
    def _optimizer_loop(self, group: str) -> TensorDictBase:
        ...
        for loss_name, loss_value in loss_vals.items():
            if loss_name in self.optimizers[group].keys():
                ...
                info = optimizer.step()
                if isinstance(info, dict):  # optimizers that report diagnostics
                    for key, value in info.items():
                        training_td.set(
                            f"{key}_{loss_name}",
                            torch.tensor(value, device=self.config.train_device),
                        )
                optimizer.zero_grad()
    ...
    def _backward_all_losses(self, group: str, subdata: TensorDictBase):
        """Recomputes every loss of ``group`` on ``subdata`` and fills ``.grad``.

        Returns the raw (pre-``process_loss_vals``) loss tensordict, so that the
        caller can log exactly what the single-gradient loop logs.
        """
        for optimizer in self.optimizers[group].values():
            optimizer.zero_grad()
        loss_vals = self.losses[group](subdata)
        raw = loss_vals.detach()
        loss_vals = self.algorithm.process_loss_vals(group, loss_vals)
        for loss_name, loss_value in loss_vals.items():
            if loss_name in self.optimizers[group].keys():
                loss_value.backward()
    ...
    def _optimizer_loop_two_point(self, group: str) -> TensorDictBase:
        """Optimizer loop for methods needing the gradient at a probe point.

        Two forward/backward passes on the **same** minibatch, no matter how many
        losses the group has:

        1. gradient at ``u_k``
        2. every optimizer stores ``u_k``, ``F(u_k)`` and moves its parameters to
           its probe point ``v_k``
        3. gradient at ``v_k``
        4. every optimizer restores ``u_k`` and applies its update

        Step 2 moves the parameters of *all* the losses at once, which is only
        equivalent to moving them one at a time because the losses do not share
        parameters and do not read each other's (``loss_objective`` uses the
        advantage precomputed in ``process_batch``, not the live critic).
        :meth:`_check_two_point_optimizers` enforces the first condition.
        """
        subdata = self.replay_buffers[group].sample().to(self.config.train_device)
        optimizers = self.optimizers[group]

        # 1. F(u_k)
        training_td = self._backward_all_losses(group, subdata)
        for loss_name, optimizer in optimizers.items():
            training_td.set(
                f"grad_norm_{loss_name}",
                torch.tensor(
                    self._grad_norm(optimizer), device=self.config.train_device
                ),
            )

        # 2. move to the probe points
        for optimizer in optimizers.values():
            optimizer.probe()

        # 3. F(v_k)
        try:
            self._backward_all_losses(group, subdata)
        except Exception:
            for optimizer in optimizers.values():
                optimizer.restore()
            raise
    ...
    def _grad_norm(self, optimizer: torch.optim.Optimizer) -> float:
        """The global gradient norm of ``optimizer``'s parameters, without clipping."""
        params = []
        for param_group in optimizer.param_groups:
            params += param_group["params"]
        norms = [
            torch.linalg.vector_norm(p.grad, 2.0)
            for p in params
            if p.grad is not None
        ]
        if not len(norms):
            return 0.0
        return float(torch.linalg.vector_norm(torch.stack(norms), 2.0))
    ...
    def state_dict(self) -> OrderedDict:
        ...
        state_dict.update(
            {
                f"optimizer_{group}_{loss_name}": optimizer.state_dict()
                for group, optimizers in self.optimizers.items()
                for loss_name, optimizer in optimizers.items()
            }
        )
    ...
    def load_state_dict(self, state_dict: Dict) -> None:
        ...
        for group, optimizers in self.optimizers.items():
            for loss_name, optimizer in optimizers.items():
                key = f"optimizer_{group}_{loss_name}"
                if key in state_dict:  # checkpoints from before optimizers were saved
