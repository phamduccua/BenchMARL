...
def load_experiment_from_hydra(
    ...
    optimizer_config = (
        load_optimizer_config_from_hydra(cfg.optimizer)
...
def load_optimizer_config_from_hydra(cfg: DictConfig) -> OptimizerConfig:
    """Returns a :class:`~benchmarl.optimizers.OptimizerConfig` from hydra config.

    Args:
        cfg (DictConfig): the optimizer config dictionary from hydra

    Returns:
        :class:`~benchmarl.optimizers.OptimizerConfig`
...
def reload_experiment_from_file(restore_file: str) -> Experiment:
    ...
    for key in ("experiment", "algorithm", "task", "model", "critic_model", "optimizer"):
        if key not in cfg_loaded:
            # checkpoints taken before optimizers became configurable
            continue
